...
import {AppState, Command, GameStatus, Player, reducer, TileType} from "./reducers/appReducer.ts";
...
function App() {
  ...
  return (
      <div className={"flex flex-col justify-center items-center w-screen h-screen bg-custom-bg"}>
        ...
        {!(gameStatus === GameStatus.PLAYING) ?
            <button className={"border-custom-border border-2 text-custom-text px-2 text-2xl bg-custom-primary hover:bg-custom-highlight rounded-lg"} onClick={handleResetOnClick}>Play Again?</button>
            :
