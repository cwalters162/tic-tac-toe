...
export function reducer(state: AppState, action: Action) {
    ...
    function checkForThreeInARow(tileType: TileType, checkState: AppState) {
        for (let i = 0; i < 3; i++) {
            if (checkState.gameBoard[i][0] === tileType && checkState.gameBoard[i][1] === tileType && checkState.gameBoard[i][2] === tileType) {
                return true
            }
            if (checkState.gameBoard[0][i] === tileType && checkState.gameBoard[1][i] === tileType && checkState.gameBoard[2][i] === tileType) {
                return true
            }
        }

        if (checkState.gameBoard[0][0] === tileType && checkState.gameBoard[1][1] === tileType && checkState.gameBoard[2][2] === tileType) {
            return true
        }

        return checkState.gameBoard[0][2] === tileType && checkState.gameBoard[1][1] === tileType && checkState.gameBoard[2][0] === tileType;
    }
